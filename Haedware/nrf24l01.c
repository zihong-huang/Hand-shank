...
const u8 TX_ADDRESS_M[TX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //发送地址
const u8 RX_ADDRESS_M[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址					    
const u8 RX_ADDRESS_S[RX_ADR_WIDTH]={0x44,0x43,0x10,0x10,0x04}; //接收地址	
...
//CE->PB0,CSN->PA7,SCK->PA4,MOSI->PA6,MISO->PA5,IRQ->PA12
void Init_NRF24L01(void)
{
	//CSN->PA7,SCK->PA4,MOSI->PA6
...
void RX_Mode(void)
{
	CE_L;	  
    //写RX节点地址
  	NRF24L01_Write_Buf(SPI_WRITE_REG+RX_ADDR_P0,(u8*)RX_ADDRESS_M,RX_ADR_WIDTH);
...
{														 
	CE_L;	    
    //写TX节点地址 
  	NRF24L01_Write_Buf(SPI_WRITE_REG+TX_ADDR,(u8*)TX_ADDRESS_M,TX_ADR_WIDTH);    
    //设置TX节点地址,主要为了使能ACK	  
  	NRF24L01_Write_Buf(SPI_WRITE_REG+RX_ADDR_P0,(u8*)RX_ADDRESS_M,RX_ADR_WIDTH); 
